...
while getopts i:s:d:l:N: opt; do
  case "$opt" in
      i) BamFil="$OPTARG";;
      s) Settings="$OPTARG";;
      d) RalDir="$OPTARG";;
...
#Set local variables
JobNm=${JOB_NAME#*.}
TmpDir=$BamFil.GenBQSR.$NCores
mkdir -p $TmpDir
RclTable=$BamFil.$NCores.recal.table
RalFils=$BamFil.$NCores.realignedfile.list
...
cmd="$JAVA7BIN -Xmx7G -Djava.io.tmpdir=$TmpDir -jar $GATKJAR -T BaseRecalibrator -R $REF -L $TARGET $RclInputArg -knownSites $DBSNPf -knownSites $INDEL -o $RclTable -nct $NCores"
echo "    "$cmd >> $LogFil
$cmd
echo "----------------------------------------------------------------" >> $LogFil
echo ""
