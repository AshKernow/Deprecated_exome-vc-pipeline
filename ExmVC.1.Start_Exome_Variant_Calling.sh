...
if [[ $VCFTool == "HC" ]]; then
	cmd="qsub -t 1:$NumJobs -pe smp 6 -l $vcHapCExmAlloc -N vcHapCExm.$JobNm $EXOMSCR/ExmVC.2hc.HaplotypeCaller.sh -i $BamLst -s $Settings -l $LogFil -n 6 -j $NumJobs"
	...
	cmd="qsub -t 1:$NumJobs -pe smp 6 -l $vcUniGExmAlloc -N vcUniGExm.$JobNm $EXOMSCR/ExmVC.2ug.UnifiedGenotyper.sh -i $BamLst -s $Settings -l $LogFil -n 6 -j $NumJobs"
